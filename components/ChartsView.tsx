...
// Since Chart.js is from a CDN, we need to declare the global Chart object for TypeScript
declare global {
    interface Window {
        // The UMD build from the CDN makes the Chart constructor globally available.
        // It auto-registers all components, so there is no `registerables` property.
        Chart: typeof Chart;
    }
}
...
const ChartsView: React.FC<ChartsViewProps> = ({ chartData }) => {
    ...
    useEffect(() => {
        if (!window.Chart || !timeCanvasRef.current || !ratioCanvasRef.current) return;
            ...
            timeChartRef.current = new ChartJS(timeCtx, {
                ...
                options: { ...commonOptions, 
                  plugins: { ...commonOptions.plugins, tooltip: {...commonOptions.plugins.tooltip, callbacks: {
                    label: (context) => `${context.dataset.label}: ${context.parsed.y.toFixed(4)}s`
                  }}},
