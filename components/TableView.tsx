...
const TableView: React.FC<TableViewProps> = ({ results, chartData }) => {
  ...
  const getRatioSignificance = () => {
    ...
    if (results.Approximation_Ratio > gwRatio) {
  ...
  const getTimeSignificance = () => {
    if (dataIndex === -1) return `Targeting ~${results.Target_Time_s}s (Run on this CPU)`;
    ...
    const isFasterThanGW = results.Target_Time_s < gwTime;
    const isFasterThanQA = results.Target_Time_s < qaTime;
    ...
    if (isFasterThanGW) {
     ...
     return <span><span className="font-bold text-white">Targeting ~{results.Target_Time_s}s</span> (Run on this CPU)</span>
  };
    ...
    <div className="w-full">
      <div className="bg-black/20 p-4 sm:p-6 rounded-lg">
        ...
        <MetricRow 
          ...
          value={results.Approximation_Ratio.toFixed(3)}
          significance={getRatioSignificance()}
          isHighlighted={true}
        />
        <MetricRow 
          metric="Execution Time"
          value={`${results.Execution_Time_s.toFixed(4)}s`}
          significance={getTimeSignificance()}
          isHighlighted={true}
        />
        <MetricRow 
          metric="Gradient Stability (||∇S||)"
          value={results.Gradient_Stability.toExponential(1)}
        ...
        <MetricRow 
          ...
          value={results.Cut_Value.toFixed(2)}
      ...
      <div className="mt-6 text-center bg-brand-secondary/30 text-brand-primary p-4 rounded-lg">
        <span className="font-bold">Status Note:</span> {results.Scalability_Note}
      </div>
    </div>
  );
};
